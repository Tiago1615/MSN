function poconbe_cc(infile,outfile)
  % POCONBE_CC 2D BEM solver with constant elements for heat conduction problem.
  %
  ...
  %
  %   POCONBE_cc(infile,outfile) reads the model from file "infile" and write
  ...
  [Ta,mk,tcc,vcc,nx,en,ec,ipx,xf,nf]=read_model(infile);
  
  % Plot mesh
  plot_mesh(tcc,nx,en,ec,ipx,xf)
  ...
  [A,b]=build(Ta,mk,tcc,vcc,nx,en,ec,xf,nf);
  ...
  write_results(outfile,Ta,mk,xf,nT,nq,ipx,ipT,ipq);  
