...
%   See also POCONBE, UNMAP, WRITE_RESULTS.
  ...
  for kj=1:ne
    ...
    for ki=1:nip
      ...
      ipT(ki)=ipT(ki)+dot(g,nq([kj1 kj2]))-dot(h,nT([kj1 kj2]));
      ipq(1,ki)=ipq(1,ki)+dot(l(1,:),nq([kj1 kj2]))-dot(m(1,:),nT([kj1 kj2]));
      ipq(2,ki)=ipq(2,ki)+dot(l(2,:),nq([kj1 kj2]))-dot(m(2,:),nT([kj1 kj2]));
    end
  end
